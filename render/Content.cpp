...
Content::Content(HWND hwnd) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        this->m_writeFac->CreateTextFormat(L"微软雅黑", NULL, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 16, L"en-US", &this->m_forMat);
...
void Content::fill() { this->m_render->FillGeometry(this->m_path, this->brush); }
void Content::releasePath() {
    SafeRelease(&(this->m_slink));
    SafeRelease(&(this->m_path));
}
void Content::strokeStyle(UINT32 hex, FLOAT alpha) { this->pen->SetColor(D2D1::ColorF(hex, alpha)); }
...
void Content::beginPath(int x, int y) { this->m_slink->BeginFigure(D2D1::Point2F(x, y), D2D1_FIGURE_BEGIN_FILLED); }
...
void Content::addPoint(int x, int y) { this->m_slink->AddLine(D2D1::Point2F(x, y)); };
void Content::addPoint(D2D1_POINT_2F point) { this->m_slink->AddLine(point); }

void Content::addBezier(D2D1_POINT_2F start, D2D1_POINT_2F center, D2D1_POINT_2F end) { this->m_slink->AddBezier(D2D1::BezierSegment(start, center, end)); };
...
void Content::save() { this->bufM1 = this->m1; }
...
void Content::drawImage(ID2D1Bitmap* img, int x, int y, int zoom) { this->drawImage(img, x, y, zoom, zoom); }
