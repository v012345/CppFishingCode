...
bool DisplayObject::inRect(POINT* mouse) { return (mouse->x >= this->g_x && mouse->x <= this->g_x + this->getWidth() && mouse->y > this->g_y && mouse->y <= this->g_y + this->getHeight()); }
...
void DisplayObject::setId(string id) { this->id = id; }
...
void DisplayObject::initG_x() {
    ...
    for (unsigned int i = 0; i < this->children.size(); i++) { this->children[i]->initG_x(); }
};
void DisplayObject::initG_y() {
    ...
    for (unsigned int i = 0; i < this->children.size(); i++) { this->children[i]->initG_y(); }
};
...
float DisplayObject::getWidth() { return this->width * this->zoom; }
float DisplayObject::getHeight() { return this->height * this->zoom; }
DisplayObject::~DisplayObject() {}
void DisplayObject::render(DemoApp* app) {
    if (!this->visible) { return; }
    for (unsigned int i = 0; i < this->children.size(); i++) { this->children[i]->render(app); }
};