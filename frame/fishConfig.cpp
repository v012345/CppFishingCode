...
fishConfig::fishConfig(App* app, action* scene, CollisionManger* colObj, Sprite* img, std::function<void(fish* b)> removeScene) {
...
void fishConfig::frameFun(vector<fish*>* fishArr, DisplayObject* ds) {
    ...
    if (this->createTimmer <= 0) {
        ...
        for (int i = 0; i < this->createNum; i++) { this->createFish(fishArr, ds); }
    } else {
        this->createTimmer--;
    }
};
