...
void files::getFile(LPWSTR url) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) { MessageBox(NULL, L"错误", L"txt", MB_OK); }
};
void files::close() { CloseHandle(this->hFile); }
void files::size() { this->len = GetFileSize(this->hFile, NULL); }
void files::readFile() {
    this->size();
    if (!ReadFile(this->hFile, this->lp, this->len, &this->len2, NULL)) { MessageBox(NULL, L"错误", L"txt", MB_OK); };
    stringstream ss;
    ss << this->lp;
    this->s = ss.str();
...
void files::writeFile() {
    ...
    if (len3 < this->len) { len3 = this->len; }
