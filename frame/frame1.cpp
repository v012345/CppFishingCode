...
void frame1::init(DemoApp** app, colVec** colObj) {
    ...
    if (frame1::hThred == NULL) { MessageBox(NULL, L"asd", L"asd", MB_OK); }
...
fishConfig* frame1::initFishConfig(LPWSTR url, int swimY, int w, int h, int swimNum, int deathY, int deathNum, int getGold, int maxFish, int deathInt) {
    fishConfig* fc1 = new fishConfig(frame1::app, frame1::scene, frame1::colObj, new sprite(frame1::app, url), frame1::removeFish);
...
void frame1::drawGoldNum() {
    ...
    int len = a1.length();
    if (len > 6) {
        for (int d = 6; d < len; d++) { frame1::jbNum[d]->visible = true; }
    } else {
        for (int d = 6; d < 10; d++) { frame1::jbNum[d]->visible = false; }
    }
    for (int i = len - 1, q = 0; i >= 0; i--, q++) {
        int a2 = a1[q] - 48;
        frame1::jbNum[i]->frame->x = a2 * 36;
    }
    for (int e = len; e < 9; e++) { frame1::jbNum[e]->frame->x = 0; }
};
void frame1::tk1FunC(DemoApp** app, ticker* tk) {
    frame1::timerFish += 15;
    unsigned int i = 0;
    unsigned int len = frame1::fishConfigArr.size() - 5 + frame1::nowScane;
    for (i = 0; i < len; i++) { frame1::fishConfigArr[i]->frameFun(&frame1::fishArr, frame1::fishBox); }
    for (i = 0; i < frame1::fishArr.size(); i++) {
        if (frame1::timerFish >= 200) {

            if (!frame1::fishArr[i]->inScreen()) { frame1::fishArr[i]->changeType("death"); }
            if (i == len - 1) { frame1::timerFish = 0; }
        }
        frame1::fishArr[i]->frameFun();
    }
    for (i = 0; i < frame1::goldArr.size(); i++) { frame1::goldArr[i]->frameFun(); }
    for (i = 0; i < frame1::bulletArr2.size(); i++) {
        if (frame1::bulletArr2[i]->bulletType == "bt") { frame1::bulletArr2[i]->moveForword(); }
    }
    frame1::drawGoldNum();
}
DWORD WINAPI frame1::tk1Fun() {
    ...
    while (true) {
        ...
        unsigned int i = 0;
        for (i = 0; i < frame1::bulletArr2.size(); i++) {
            if (frame1::bulletArr2[i]->frameFun(&frame1::fishArr) == 1) { i--; };
        }
    }
...
void frame1::onClick(int x, int y) {
    ...
    bullet* b = new bullet(frame1::app, frame1::scene, frame1::colObj, frame1::bottom["bullet"], frame1::bottom["wang"]);
...
void frame1::onMouseDown(int x, int y) {
    frame1::isDown = true;
    if (frame1::addBtn->inRect(frame1::app->mouse)) { frame1::addBtn->img = frame1::app->content->getSoucre(L"img/Interface/cannon_plus_down.png"); }
    if (frame1::subtractBtn->inRect(frame1::app->mouse)) { frame1::subtractBtn->img = frame1::app->content->getSoucre(L"img/Interface/cannon_minus_down.png"); }
};
