...
bullet::~bullet() {}
bullet::bullet(App* app, action* scene, CollisionManger* colObj, Sprite* bt, Sprite* wang, int hurt, float speed) {
    this->app = (app);
    this->scene = (scene);
    this->colObj = (colObj);
    this->bt = bt;
...
void bullet::moveForword() {
    utils::usePoint ps = {this->view->g_x, this->view->g_y};
...
void bullet::setX(float x) { this->view->setX(x); }
void bullet::setY(float y) { this->view->setY(y); }